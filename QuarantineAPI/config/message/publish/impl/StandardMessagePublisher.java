...
import QuarantineAPI.config.message.result.impl.StandardMessagePublicationResult;
...
public final class StandardMessagePublisher<T> implements MessagePublisher<T> {
  public MessagePublicationResult<T> publish(T topic, AbstractAsynchronousPubSubMessageBus<T> messageBus) {
    ...
    return (messageHandlers == null) ? (MessagePublicationResult<T>)new DeadMessagePublicationResult() : (MessagePublicationResult<T>)new StandardMessagePublicationResult((AsynchronousPublishMessageBus)messageBus, messageHandlers, topic);
  }
}
