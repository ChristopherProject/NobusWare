package it.nobusware.client.render.loup.server.checker;
...
public class GuiPortChecker extends GuiScreen {
    ...
    if (clickedButton.enabled)
      if (clickedButton.id == 0) {
        (new Thread("ports") {
            public void run() {
              try {
                for (int i = Integer.parseInt(GuiPortChecker.this.maxThreadsBox.getText()); i <= Integer.parseInt(GuiPortChecker.this.portBox.getText()); i++) {
                  String serverip = String.valueOf(GuiPortChecker.this.ipBox.getText()) + ":" + i;
                  OldServerPinger pinger = new OldServerPinger();
                  try {
                    pinger.ping(new ServerData("§aPort Checking " + i, serverip));
                    GuiPortChecker.this.prevMenu.savedServerList.addServerData(new ServerData("§aPort Checking " + i, serverip));
                    GuiPortChecker.this.working = GuiPortChecker.this.working + 1;
                  } catch (UnknownHostException unknownHostException) {
                  
                  } catch (Exception exception) {}
                  pinger.clearPendingNetworks();
                  GuiPortChecker.this.checked = GuiPortChecker.this.checked + 1;
                  GuiPortChecker.this.prevMenu.savedServerList.saveServerList();
                  GuiPortChecker.this.prevMenu.serverListSelector.func_148195_a(GuiPortChecker.this.prevMenu.savedServerList);
                } 
              } catch (Exception exception) {}
            }
          }).start();
  ...
  public void drawScreen(int par1, int par2, float par3) {
    ...
    drawCenteredString(this.fontRendererObj, "Port checker", this.width / 2, 20, 
        16777215);
    drawCenteredString(this.fontRendererObj, 
        "This will search for servers in the port range", this.width / 2, 40, 
        10526880);
    drawCenteredString(this.fontRendererObj, 
