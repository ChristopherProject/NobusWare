package it.nobusware.client.utils.fontmanager.other;
...
public class DefaultFont {
  ...
  private void drawChar(char character, float x, float y) throws ArrayIndexOutOfBoundsException {
    ...
    drawTexturedModalRect(x, y, this.xPos[character - this.startChar], this.yPos[character - this.startChar], (float)bounds.getWidth() + 1.0F, (float)bounds.getHeight() + this.theMetrics.getMaxDescent() + 1.0F);
  }
  ...
  private void drawer(String text, float x, float y, int color) {
    ...
    for (int i = 0; i < text.length(); i++) {
      if (text.charAt(i) == 'ง' && i + 1 < text.length()) {
  ...
  private String getFormatFromString(String par0Str) {
    ...
    while ((var2 = par0Str.indexOf('ง', var2 + 1)) != -1) {
      if (var2 < var3 - 1) {
        ...
        if (!isFormatSpecial(var4))
          continue; 
        var1 = String.valueOf(String.valueOf(var1)) + "ง" + var4;
      } 
    } 
    return var1;
  }
  ...
  public String wrapFormattedStringToWidth(String s, float width) {
    ...
    String split2 = String.valueOf(String.valueOf(getFormatFromString(split))) + s.substring(wrapWidth + ((s.charAt(wrapWidth) == ' ' || s.charAt(wrapWidth) == '\n') ? 1 : 0));
    try {
