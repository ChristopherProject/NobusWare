...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void func_175161_p() {
    	EventUpdate event = new EventUpdate(this.posX, this.boundingBox.minY,
		...
		if (var1 != this.field_175171_bO) {
			if (var1) {
				this.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity) this,
						C0BPacketEntityAction.Action.START_SPRINTING));
			} else {
				this.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity) this, C0BPacketEntityAction.Action.STOP_SPRINTING));
			}
		...
		if (var2 != this.field_175170_bN) {
			if (var2) {
				this.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity) this, C0BPacketEntityAction.Action.START_SNEAKING));
			} else {
				this.sendQueue.addToSendQueue(new C0BPacketEntityAction((Entity) this, C0BPacketEntityAction.Action.STOP_SNEAKING));
			}
		...
		if (func_175160_A()) {
			...
			if (this.ridingEntity == null) {
				if (var13 && var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getX(),
							event.getY(), event.getZ(), event.getYaw(), event.getPitch(), event.getGroundState()));
				} else if (var13) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.getX(),
							event.getY(), event.getZ(), event.getGroundState()));
				} else if (var14) {
					this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event.getYaw(),
							event.getPitch(), event.getGroundState()));
				} else {
					this.sendQueue.addToSendQueue(new C03PacketPlayer(event.getGroundState()));
				}
			} else {
    ...
    {
    	EventChatSend event = new EventChatSend(p_71165_1_);
    	EventAPI.fire(event);
    	if(event.isCancelled()) {
    		return;
    	}else {
	...
	public boolean isMoving() {
		return (this.movementInput.getMoveForward() != 0 || this.movementInput.getMoveStrafe() != 0);
	}
