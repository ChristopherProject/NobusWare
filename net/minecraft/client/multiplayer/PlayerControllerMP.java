...
import it.nobusware.client.mods.InfiniteBlockReach;
...
public class PlayerControllerMP {
    ...
    public float getBlockReachDistance() {
        if (mc.getNobita().getModManager().Prendi(InfiniteBlockReach.class).isAbilitato()) {
            return 200;
        }
    ...
    public boolean placeBlock(EntityPlayerSP player, WorldClient worldClient, ItemStack itemStack, BlockPos blockPos, EnumFacing facing, Vec3 vec3) {
        this.syncCurrentPlayItem();
        float x = (float) (vec3.xCoord - (double) blockPos.getX());
        float y = (float) (vec3.yCoord - (double) blockPos.getY());
        float z = (float) (vec3.zCoord - (double) blockPos.getZ());
        boolean var10 = false;
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(blockPos)) {
            ...
            if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
                IBlockState var11 = worldClient.getBlockState(blockPos);

                if ((!player.isSneaking() || player.getHeldItem() == null) && var11.getBlock().onBlockActivated(worldClient, blockPos, var11, player, facing, x, y, z)) {
                    var10 = true;
                }

                if (!var10 && itemStack != null && itemStack.getItem() instanceof ItemBlock) {
                    ItemBlock var12 = (ItemBlock) itemStack.getItem();

                    if (!var12.canPlaceBlockOnSide(worldClient, blockPos, facing, player, itemStack)) {
                        return false;
                    }
                }
            }

            this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(blockPos, facing.getIndex(), player.inventory.getCurrentItem(), x, y, z));

            if (!var10 && this.currentGameType != WorldSettings.GameType.SPECTATOR) {
                if (itemStack == null) {
                    return false;
                } else if (this.currentGameType.isCreative()) {
                    int var14 = itemStack.getMetadata();
                    int var15 = itemStack.stackSize;
                    boolean var13 = itemStack.onItemUse(player, worldClient, blockPos, facing, x, y, z);
                    itemStack.setItemDamage(var14);
                    itemStack.stackSize = var15;
                    return var13;
                } else {
                    return itemStack.onItemUse(player, worldClient, blockPos, facing, x, y, z);
                }
            } else {
                return true;
            }
