...
import it.nobusware.client.AI.NobusWareMessageAI;
import it.nobusware.client.NobusWare;
import it.nobusware.client.events.EventJoinWorld;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void addToSendQueue(Packet p_147297_1_) {
    	EventPackets ep = new EventPackets(p_147297_1_);
        EventAPI.fire(ep);
        if (ep.isCancelled())
          return; 
        this.netManager.sendPacket(ep.getPacket());
      }
      
      public void noEventPacket(Packet p_147297_1_) {
    ...
    private boolean validateResourcePackUrl(String url, String hash) {
        try {
          ...
          Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("§7[§4Alets§7] §CThis server is using the the 1.8 resource exploit:"));
          Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("§7[§eINFO§7] §a" + url));
          return false;
        }
      }
    ...
    public void func_175095_a(S48PacketResourcePackSend p_175095_1_)
    {
        ...
        if (var2.startsWith("level://") && validateResourcePackUrl(var2, var3))
        {
    ...
    public void handleCustomPayload(S3FPacketCustomPayload packetIn)
    {
        ...
        else if ("MC|Brand".equals(packetIn.getChannelName()))
        {
	        ...
	        for (final Module m : Minecraft.getMinecraft().getNobita().getModManager().getHackDelClient()) {
	            if (m.isDangerous()) {
	                m.setAbilitato(false);
	            }
	        }
			String channels = packetIn.getBufferData().toString(Charsets.UTF_8);
			String[] channel = channels.split("\000");
			for (int i = 0; i < channel.length; i++) {
				NobusWareMessageAI.send("§e" + channel[i]);
				ArrayList<String> rattlydown = (ArrayList<String>) FileUtils.readFile(NobusWare.nobitaDir + File.separator + "channels.txt");
				rattlydown.add(channel[i]);
				FileUtils.writeFile("channels", rattlydown);
				if (channel[i].equals("BAuthMeBridge"))
					NobusWareMessageAI.send("§aThe server is using AuthMeBridge!");
			}
		}
